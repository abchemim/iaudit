...
import { MoreHorizontal, Edit, TrendingUp, Trash2 } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
import { SimplesLimit, useDeleteSimplesLimit } from "@/hooks/useSimplesLimits";
import { Progress } from "@/components/ui/progress";
import { SimplesEditDialog } from "./SimplesEditDialog";
...
export const SimplesTable = ({
  ...
  const [editLimit, setEditLimit] = useState<SimplesLimit | null>(null);
  const [editMode, setEditMode] = useState<"revenue" | "limit">("revenue");
  const deleteLimit = useDeleteSimplesLimit();
  ...
  const handleOpenEdit = (limit: SimplesLimit, mode: "revenue" | "limit") => {
    setEditLimit(limit);
    setEditMode(mode);
  };
  ...
  return (
    <>
                  ...
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={() => handleOpenEdit(limit, "revenue")}>
                          <TrendingUp className="w-4 h-4 mr-2" />
                          Atualizar Faturamento
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => handleOpenEdit(limit, "limit")}>
      ...
      <SimplesEditDialog
        limit={editLimit}
        open={!!editLimit}
        onOpenChange={(open) => !open && setEditLimit(null)}
        mode={editMode}
      />
    </>
  );
