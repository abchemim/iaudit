    ...
    queryFn: async () => {
      ...
      if (declarations) {
        ...
        declarations.forEach(decl => {
          ...
          if (typeName.toLowerCase().includes(searchQuery.toLowerCase()) || clientName.toLowerCase().includes(searchQuery.toLowerCase())) {
            results.push({
              id: decl.id,
              type: "declaracao",
      ...
      if (certidoes) {
        ...
        certidoes.forEach(cert => {
          ...
          if (cert.tipo.toLowerCase().includes(searchQuery.toLowerCase()) || orgaoName.toLowerCase().includes(searchQuery.toLowerCase()) || clientName.toLowerCase().includes(searchQuery.toLowerCase()) || "cnd".includes(searchQuery.toLowerCase()) || "certidão".includes(searchQuery.toLowerCase()) || "certidao".includes(searchQuery.toLowerCase())) {
            results.push({
              id: cert.id,
              type: "certidao",
  ...
  return <>
      <header className="h-16 border-b border-border bg-card/50 backdrop-blur-xl flex items-center justify-between px-6">
        ...
        <div className="flex items-center gap-4 mx-0 px-0 pl-[240px] ml-0">
          ...
          <input type="text" placeholder="Buscar clientes, tarefas, declarações, certidões..." value={searchQuery} onChange={e => {
            setSearchQuery(e.target.value);
            setShowResults(true);
          }} onFocus={() => setShowResults(true)} className="w-[432px] pl-10 pr-8 py-2 bg-secondary border border-border rounded-lg text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent" />
            ...
            {showResults && searchQuery.length >= 2 && <div className="absolute top-full left-0 right-0 mt-2 bg-card border border-border rounded-lg shadow-lg z-50 overflow-hidden">
                {isSearching ? <div className="p-4 text-center text-muted-foreground text-sm flex items-center justify-center gap-2">
                    <Loader2 className="w-4 h-4 animate-spin" />
                    Buscando...
                  </div> : searchResults && searchResults.length > 0 ? <ScrollArea className="max-h-64">
                    {searchResults.map(result => <button key={`${result.type}-${result.id}`} onClick={() => handleSearchResultClick(result)} className="w-full flex items-start gap-3 p-3 hover:bg-secondary/50 transition-colors text-left border-b border-border last:border-0">
                        <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center flex-shrink-0">
                          {result.type === "client" ? <Building2 className="w-4 h-4 text-primary" /> : result.type === "tarefa" ? <ClipboardList className="w-4 h-4 text-primary" /> : result.type === "declaracao" ? <FileText className="w-4 h-4 text-primary" /> : <ShieldCheck className="w-4 h-4 text-primary" />}
                        </div>
                        ...
                        <span className="text-xs text-muted-foreground bg-secondary px-2 py-0.5 rounded">
                          {result.type === "client" ? "Cliente" : result.type === "tarefa" ? "Tarefa" : result.type === "declaracao" ? "Declaração" : "Certidão"}
                        </span>
                      </button>)}
                  </ScrollArea> : <div className="p-4 text-center text-muted-foreground text-sm">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="icon" className="relative">
                ...
                {(unreadCount ?? 0) > 0 && <span className="absolute -top-1 -right-1 w-5 h-5 bg-status-danger rounded-full text-[10px] font-bold flex items-center justify-center text-white">
            ...
            <DropdownMenuContent align="end" className="w-80">
              <div className="flex items-center justify-between px-3 py-2 border-b border-border">
                ...
                {(unreadCount ?? 0) > 0 && <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={() => markAllAsRead.mutate()}>
                    <Check className="w-3 h-3 mr-1" />
                    Marcar todas como lidas
                  </Button>}
              </div>
              <ScrollArea className="h-64">
                {recentNotifications.length === 0 ? <div className="p-4 text-center text-muted-foreground text-sm">Nenhuma notificação</div> : recentNotifications.map(notification => <DropdownMenuItem key={notification.id} className={`flex flex-col items-start gap-1 p-3 cursor-pointer ${!notification.is_read ? "bg-primary/5" : ""}`} onClick={() => {
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Input id="profile-name" value={profileName} onChange={e => setProfileName(e.target.value)} placeholder="Seu nome completo" />
            </div>

            <div className="space-y-2">
              <Label htmlFor="profile-phone">Telefone</Label>
              <Input id="profile-phone" value={profilePhone} onChange={e => setProfilePhone(e.target.value)} placeholder="(00) 00000-0000" />
            </div>

            <div className="space-y-2">
              <Label>Função</Label>
              <Input value={profile?.role ? ROLE_LABELS[profile.role as UserRole] : "Contador"} disabled className="bg-muted" />
